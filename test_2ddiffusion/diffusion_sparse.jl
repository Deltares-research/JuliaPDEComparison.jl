# Sparse-matrix 2D diffusion solver.
#
# The 2D Laplacian is applied by left- and right-multiplying the field matrix u
# with prebuilt 1D sparse second-derivative matrices:
#
#   ∇²u ≈ Dx * u + u * Dy'
#
...
Apply the scaled 2D diffusion operator to field `u` using prebuilt 1D sparse
...
- `Dx` : 1D sparse second-derivative matrix in x, size (Nx, Nx).
- `Dy` : 1D sparse second-derivative matrix in y, size (Ny, Ny).
"""
...
Run a 2D diffusion simulation using Euler-forward time integration and sparse
...
Set up and run the full 2D diffusion simulation using the sparse-matrix solver.
